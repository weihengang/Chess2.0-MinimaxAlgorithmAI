...
last_move.width(5)
directory =__file__.replace("\PyChess2.0+MinimaxAlgorithmAI (Backup File).py", "") #directory to load lichess chess images
...
class MinimaxAI:
    ...
    @staticmethod
    def minimax(piece_class, board, level): #oh shit here goes 2
        ...
        """
            ...
            max_gamma =max(list_value)
            if (MinimaxAI.gamma == "not set" or MinimaxAI.gamma > max_gamma):
                MinimaxAI.gamma =max_gamma
            return max_gamma
        """
    @staticmethod
    ...
    def evaluate(board): #evaluation strategy: material (easy), piece square tables (easy), defense amount (difficult), mobility (moderate)
        piece_value ={
            "p": [-100, MinimaxAI.PST_pawn],
            "h": [-300, MinimaxAI.PST_horse],
            "b": [-300, MinimaxAI.PST_bishop],
            "c": [-500, MinimaxAI.PST_castle],
            "q": [-900, MinimaxAI.PST_queen],
            "k": [-100000, MinimaxAI.PST_king],
            "P": [100, MinimaxAI.PST_PAWN],
            "H": [300, MinimaxAI.PST_HORSE],
            "B": [300, MinimaxAI.PST_BISHOP],
            "C": [500, MinimaxAI.PST_CASTLE],
            "Q": [900, MinimaxAI.PST_QUEEN],
            "K": [10000, MinimaxAI.PST_KING]
        }
        board_value =0
        for i in board:
            for j in i:
                if (j in piece_value):
                    try:
                        ...
                        board_value += piece_value[j][0] #original (alpha-beta pruning and piece square tables wont have any effect)
        return board_value
